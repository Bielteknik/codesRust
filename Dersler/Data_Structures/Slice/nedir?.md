Rust'ta **Slice** (Türkçesiyle *Dilim*), bir veri koleksiyonunun (Array, Vec, String) **tamamına veya belirli bir ardışık kısmına referans veren** (ödünç alan/borrow eden) bir veri yapısıdır. 

Slice'ların en büyük avantajı, veriyi kopyalamadan (memory allocation yapmadan) onun sadece bir "penceresini" açabilmenizdir.

İşte Rust Slice yapısının detaylı anlatımı:
...
Rust'ta en çok karşılaşılan slice türü **String Slice**'dır. Bir `String`'in (veya string literal'ın) belirli bir kısmına referans verir. Tipi **`&str`**'dir.
...
⚠️ **Çok Önemli Not (UTF-8 Sınırı):** Rust'ta string dilimleri **karakter** üzerinden değil, **byte** üzerinden kesim yapar. Eğer bir dilim, çok byte'lı bir karakterin (örneğin 'ü', 'ş', 'ğ' veya emoji) tam ortasından kesilirse program **panic** vererek çöker.
```rust
...
Slice'lar aslında birer **referans** (`&`) olduğu için Rust'ın katı Borrowing (Ödünç Alma) kurallarına tabidirler.
...
Rust'ta normal bir referans (örneğin `&i32`) sadece bellekteki adresi tutar (8 byte - 64 bit sistemde). Ancak bir Slice (`&[T]`), arka planda bir **Fat Pointer (Şişkin İşaretçi)** kullanır. İki şeyi birden tutar:
1. Verinin başladığı **bellek adresi** (Pointer).
2. Dilimin **uzunluğu** (Length).

Bu sayede Rust, fonksiyona sadece dilimi geçirseniz bile, dilimin kaç elemanlı olduğunu her zaman bilir.
...
* **Fonksiyon Parametreleri:** Bir fonksiyon yazarken parametreyi `&Vec<T>` yerine `&[T]` (veya String için `&String` yerine `&str`) olarak tanımlamak Rust'ta **en iyi pratiktir (best practice)**.
* **Sınır Kontrolleri:** Dilim alırken `[0..100]` gibi sınırları aşarsanız program derlenmez değil, **çalışma anında (runtime) panic vererek çöker**. Rust, bellek güvenliği için sınırları her zaman kontrol eder.
